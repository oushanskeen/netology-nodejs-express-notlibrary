...
class BooksRepository {
    constructor() {
        this.createBook = (book) => db.get("books").push(newBook).write();
        this.getBook = (id) => db.get("books").find({ id: id }).value();
        this.getBooks = () => db.get("books").value();
        this.updateBook = (id, newValue) => db.set(`books.${id}`, newValue).write();
        this.deleteBook = (id) => db.get("books").remove({ id: id }).write();
    }
}
const downloadBook = (req, res) => {
    const fileId = req.params.id;
    const directoryPath = /*__dirname*/ "./downloads";
    res.download(directoryPath + `/book${fileId}.txt`, `book${fileId}.txt`, err => {
        err
...
const createBook = (req, res) => {
    ...
    const newLib = [...dbBooks, Object.assign({}, newBook)];
...
const updateBook = (req, res) => {
    ...
    const newBook = Object.assign({ id: req.params.id }, req.body);
    //db.get("books");
